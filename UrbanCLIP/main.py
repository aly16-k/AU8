...
def create_args():
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default=f"{city}_captions",
        choices=[f"{city}_captions"],
        help="which dataset",
    )
    parser.add_argument(
        "--lr", type=float, default=0.0001, help="learning rate"
    )  # very sensitive  0.0003
    parser.add_argument(
    ...
    parser.add_argument(
        "--pretrained_model",
        type=str,
        default="laion-mscoco_finetuned_CoCa-ViT-L-14-laion2B-s13B-b90k/open_clip_pytorch_model.bin",
...
def create_datasets(args, transform, tokenizer):
    ...
    if args.dataset == f"{city}_captions":
        data = json.load(open(f"data/captions/{city}_captions.json", "r", encoding="utf-8"))
    else:
...
def main():
    ...
    logger.remove()  # 清掉默认的
    logger.add(sys.stderr, level="INFO")  # 输出到 console
    logger.add(os.path.join(args.logging_dir, city + ".log"), level="INFO")
    logger.info(args)
    ...
    num_workers=8, pin_memory=True  # !!! new
    )
    ...
    for epoch in tqdm(range(args.epoch_num), desc="Training"):
        ...
        if cur_metrics["clip_val_loss"] < best_clip_val_loss:
            ...
            #     os.path.join(args.checkpoint_dir, f"best_states_{city}.pt"),
            # )
            torch.save(
                model.state_dict(),
                # os.path.join(args.checkpoint_dir, f"epoch_{completed_epoch}.pt"),
                os.path.join(args.checkpoint_dir, f"best_model_{city}.bin"),
            )
