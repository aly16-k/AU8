...
# ---------------------------
# 1.1 Reproducibility (Global seed and determinism)
# ---------------------------
SEED = 42

def set_seed(seed: int = 42, cuda_deterministic: bool = True):
    # Execute as early as possible in the script
    os.environ["PYTHONHASHSEED"] = str(seed)
    # Required for some CUDA kernels (only when use_deterministic_algorithms=True)
    ...
    # CuDNN determinism (disable benchmark)
    torch.backends.cudnn.benchmark = False
    torch.backends.cudnn.deterministic = True

    # Stronger determinism (unsupported ops will throw an error;
    # set to False or wrap with try/except if you don’t want exceptions)
    try:
...
# ---------------------------
# 2. Hybrid CNN-ViT Model (Aligned with training architecture and hyperparameters)
# ---------------------------
...
class Tile2VecHybrid(nn.Module):
    ...
    - cnn_stem: 68 -> 34 -> 17 -> 16 (last layer k=2, s=1, p=0)
    - patch_size=4 → 16×16 → 4*4=16 patches; +CLS = 17 tokens
    """
    def __init__(self, image_size=68, patch_size=4, dim=256, depth=4, heads=4,
        ...
        feature_size = image_size // 4  # 17 (for pos_embedding upper bound; actual size is 16)
...
# ---------------------------
# 3. Device & embedding dimension
# ---------------------------
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
embedding_dim = 128

# ---------------------------
# 4. Image loading (Normalization/size/channels consistent with training)
# ---------------------------
def _load_img_as_tensor_infer(path):
    """
    - GeoTIFF (uint16, S2 SR): /10000 → clamp[0,1], ensure (4,H,W), size 68×68
    - PNG/JPG compatible: RGB/255, padded to 4 channels; size 68×68
    """
    if path.lower().endswith(('.tif', '.tiff')):
        ...
        # Channel handling: truncate/pad to 4
        if img.shape[0] > 4:
            img = img[:4]
        elif img.shape[0] < 4:
        ...
        # Resize to 68×68 (if already 68, keep unchanged)
...
def extract_features(image_folder, df, image_column, model):
    ...
    for img_name in tqdm(df[image_column], desc="Extracting features"):
        ...
        with torch.no_grad():
            feat = model(img)
        features.append(feat.cpu().numpy().astype(np.float32).flatten())  # Explicit float32
    return np.array(features, dtype=np.float32)  # XGBoost input unified as float32

# ---------------------------
# 4.1 Construct GPU + deterministic XGBoost regressor as much as possible
# ---------------------------
def make_xgb_gpu_regressor(seed: int = SEED):
    ...
    if (ver_major, ver_minor) >= (2, 0):
        # v2.x: only need device="cuda" + tree_method="hist"
        return XGBRegressor(
            **base,
            device="cuda",
            tree_method="hist"
    ...
    for city in cities:
        ...
        if os.path.exists(ckpt_path_city):
            ...
            raise FileNotFoundError(
                f"Weight file not found: {ckpt_path_city}. Please check the saved pth path."
            )
        ...
        for i in range(len(root)):  # i = 0 ~ 8
            ...
            # XGBoost: GPU + deterministic settings as much as possible (auto-select by version)
